...
class AutoDiffer:
    def __init__(self):
        self.trace_level = (
            -1
        )  # global counter of the number of functions we're currently tracing
    ...
    def _backward(self, gradout, end_result, level):
        ...
        for result in topo_sorted:
            ...
            for par_argidx, par in result.get_bprop_parents(level):
                ...
                par_vjp = op_to_vjp_func[type(result.op)](
                    par_argidx, gradout, result, all_parents
                )
    ...
    def make_grad_and_val_func(self, f):
        ...
        def grad_and_val_func(arg0, *args):
            self.trace_level += (
                1  # increment global trace to indicate we're tracing a new function
            )
            if isinstance(
                arg0, Result
            ):  # if arg0 is a Result it's already being traced
                init_result = arg0
                init_result.trace_levels.add(
                    self.trace_level
                )  # add new trace to its levels
            ...
            if end_result is None:
                assert False
            assert (
                end_result.get_val().size == 1
            ), "Can only take grads of scalar functions!!!"
