...
class Result:
    ...
    def get_all_parents(self, remove_level):
        ...
        """
        return [par.clone(remove_level) if isinstance(par, Result) else par 
    ...
    def get_bprop_parents(self, level):
        ...
        """
        return [(i, par) for i, par in enumerate(self.parents) 
...
## -------------------------------------------- OPs utilities ----------------------------------------------- ##
def _get_shape(arg):
    assert not isinstance(arg, list), "lists have no shape; arg should be numeric or an ndarray!"
...
class MyBroadcast(MyOp):
    ...
    def compute_op(self, arg0, shape, axis):
        ...
        assert isinstance(arg0, float) or len(arg0.shape) == 1, "can only broadcast floats or vectors"
        # scalar case
        ...
        assert shape[0] == ncols or shape[1] == ncols, "at least one dim must match input dim"
...
square, tanh, cosh, sinh, exp, log = MySquare(), MyTanh(), MyCosh(), MySinh(), MyExp(), MyLog()
...
def get_vjp_for_arg(argidx, gradout, result, args):
    ...
    zerolist = [np.zeros(thing.shape) if isinstance(thing, np.ndarray) else 0
                for thing in alist]
...
def bdcast_vjp_for_arg(argidx, gradout, result, args):
    assert argidx == 0
    arg0 = args[0]
    assert isinstance(arg0, float) or len(arg0.shape) == 1, "could've only broadcast floats or vectors"
